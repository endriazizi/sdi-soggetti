...
public class DomicilioServiceImpl implements DomicilioService {
    ...
    @Autowired
    private DomicilioModelMapper domicilioModelMapper;
    ...
    @Override
    public DomicilioDTO create(DomicilioCreateDTO dto) {

        System.out.println(dto);

        DomicilioDTO domicilioDTO = domicilioModelMapper.fromCreateDto(dto);

       // this.checkValidate(residenzaDTO);

        DomicilioModel domicilioModel = domicilioModelMapper.toModel(domicilioDTO);
        DomicilioModel model = domicilioPersistence.save(domicilioModel);

        return domicilioModelMapper.toDto(model);
    }

    @Override
    public List<DomicilioDTO> findByIds(List<UUID> ids) {
        List<DomicilioModel> models = domicilioPersistence.findByIds(ids);
        return domicilioModelMapper.toDto(models);
    }

    @Override
    public Page<DomicilioDTO> search(DomicilioCriteria criteria, Pageable pageRequest) {
        //criteria.setElimina((short)1);
        Page<DomicilioModel> models = domicilioPersistence.search(criteria, pageRequest);
        return models.map(model -> this.domicilioModelMapper.toDto(model));
    }
...
//	}
    ...
    @Override
    public DomicilioDTO update(DomicilioUpdateDTO dto) {
        DomicilioDTO domicilioDTO = domicilioModelMapper.fromUpdateDto(dto);
//		this.checkValidate(anagraficaDTO);
//		this.checkDomandaExists(anagraficaDTO.getId());
        DomicilioModel domicilioModel = domicilioModelMapper.toModel(domicilioDTO);
        domicilioPersistence.save(domicilioModel);
        DomicilioModel model = domicilioPersistence.findByIds(Collections.singletonList(domicilioDTO.getId()))
                .stream()
                .findAny()
                .orElse(null);
        return domicilioModelMapper.toDto(model);
...
//		this.checkDomandaExists(id);
        DomicilioDTO domicilioDTO = this.findByIds(Collections.singletonList(id))
                .stream()
                .findAny()
                .orElse(null);
        domicilioDTO.setFlagElimina((short) 1);
