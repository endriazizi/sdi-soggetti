...
import it.iccs.simeal.sdi.soggetti.application.mapper.ResidenzaStoricoModelMapper;
...
@Service
@Slf4j
public class ResidenzaStoricoServiceImpl implements ResidenzaStoricoService {

    @Autowired
    private ResidenzaStoricoModelMapper residenzaStoricoModelMapper;

    @Autowired
    private ResidenzaStoricoPersistence residenzaStoricoPersistence;


    @Override
    public ResidenzaStoricoDTO create(ResidenzaStoricoCreateDTO dto) {

        System.out.println(dto);

        ResidenzaStoricoDTO residenzaDTO = residenzaStoricoModelMapper.fromCreateDto(dto);

       // this.checkValidate(residenzaDTO);

        ResidenzaStoricoModel residenzaModel = residenzaStoricoModelMapper.toModel(residenzaDTO);
        ResidenzaStoricoModel model = residenzaStoricoPersistence.save(residenzaModel);

        return residenzaStoricoModelMapper.toDto(model);
    }

    @Override
    public List<ResidenzaStoricoDTO> findByIds(List<UUID> ids) {
        List<ResidenzaStoricoModel> models = residenzaStoricoPersistence.findByIds(ids);
        return residenzaStoricoModelMapper.toDto(models);
    }

    @Override
    public Page<ResidenzaStoricoDTO> search(ResidenzaStoricoCriteria criteria, Pageable pageRequest) {
        //criteria.setElimina((short)1);
        Page<ResidenzaStoricoModel> models = residenzaStoricoPersistence.search(criteria, pageRequest);
        return models.map(model -> this.residenzaStoricoModelMapper.toDto(model));
    }
...
//	}
    ...
    @Override
    public ResidenzaStoricoDTO update(ResidenzaStoricoUpdateDTO dto) {
        ResidenzaStoricoDTO residenzaStoricoDTO = residenzaStoricoModelMapper.fromUpdateDto(dto);
//		this.checkValidate(anagraficaDTO);
//		this.checkDomandaExists(anagraficaDTO.getId());
        ResidenzaStoricoModel residenzaStoricoModel = residenzaStoricoModelMapper.toModel(residenzaStoricoDTO);
        residenzaStoricoPersistence.save(residenzaStoricoModel);
        ResidenzaStoricoModel model = residenzaStoricoPersistence.findByIds(Collections.singletonList(residenzaStoricoDTO.getId()))
                .stream()
                .findAny()
                .orElse(null);
        return residenzaStoricoModelMapper.toDto(model);
...
//		this.checkDomandaExists(id);
        ResidenzaStoricoDTO residenzaStoricoDTO = this.findByIds(Collections.singletonList(id))
                .stream()
                .findAny()
                .orElse(null);
        residenzaStoricoDTO.setFlagElimina((short) 1);
